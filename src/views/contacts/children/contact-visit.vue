<template>
  <section class="talk">
    ...
    <van-cell-group class="van-cell-group van-cell-group--mini">
      <van-cell required title="拜访时间" :value="form.time" @click="setTime">
        <template #right-icon>
      ...
      <van-field label="拜访现场照片" required>
        <template #input>
          <van-uploader
            v-model="file"
            :max-count="1"
      ...
      <!-- eslint-disable-next-line -->
      <van-field v-model="form.digest" label="拜访摘要" placeholder="请输入拜访摘要" required />
      <van-field
