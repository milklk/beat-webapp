<template>
  <section class="talk">
    ...
    <van-cell-group class="van-cell-group van-cell-group--mini">
      ...
      <van-cell class="van-cell--auto" required title="评估时间" :value="form.time" @click="setTime">
        <template #right-icon>
          <van-icon class="van-icon" name="add-o" />
        </template>
      </van-cell>
      <!-- eslint-disable-next-line -->
      <van-field v-model="form.field" label="评估人员" placeholder="请输入评估人员" required />
      <van-field label="评估照片" required>
        <template #input>
          <van-uploader
            v-model="file"
            :max-count="1"
      ...
      <!-- eslint-disable-next-line -->
      <van-field v-model="form.digest" label="评估摘要" placeholder="请输入评估摘要" required />
      <van-field
